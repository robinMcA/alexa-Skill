...
const getOfficeInformationDevice = async ({ test }) => {
  const self = test;
...
const getOfficeInformationBussy = async ({ test }) => {
  ...
  const url = '/busyness?' + `deviceId=${friendlyName}`;
  console.log(url);
  const rawData = await officeInformationRequest(url);
  const parsedData = JSON.parse(rawData);
  const outputData = fp.flow(
  ...
  self.response
    .speak(`The bussyness of the ${self.event.request.intent.slots.room.value} is ${outputData}`)
    .listen();
  self.emit(':responseReady');
};
...
const handlers = {
  ...
  GetDevicesIntent: function() {
    getOfficeInformationDevice({ test: this });
  },
